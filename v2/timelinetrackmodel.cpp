...
namespace timeline {
	...
	void TimelineTracksModel::setTrackHidden(int row, bool hidden)
	{
		if (row < m_trackList.size()) {
			int i = m_trackList.at(row).mlt_index;
			QScopedPointer<Mlt::Producer> track(m_tractor->track(i));
	...
	bool TimelineTracksModel::addTransitionValid(int fromTrack, int toTrack, int clipIndex, int position)
	{
		bool result = false;
		int i = m_trackList.at(toTrack).mlt_index;
		QScopedPointer<Mlt::Producer> track(m_tractor->track(i));
	...
	int TimelineTracksModel::addTransition(int trackIndex, int clipIndex, int position)
	{
		int i = m_trackList.at(trackIndex).mlt_index;
		QScopedPointer<Mlt::Producer> track(m_tractor->track(i));
		if (track) {
	...
	void TimelineTracksModel::removeTransition(int trackIndex, int clipIndex)
	{
		int i = m_trackList.at(trackIndex).mlt_index;
		QScopedPointer<Mlt::Producer> track(m_tractor->track(i));
		if (track) {
	...
	void TimelineTracksModel::removeTransitionByTrimIn(int trackIndex, int clipIndex, int delta)
	{
		QModelIndex modelIndex = index(clipIndex, 0, index(trackIndex));
		clearMixReferences(trackIndex, clipIndex);
		delta = -data(modelIndex, TimelineTracksModel::DurationRole).toInt();
		liftClip(trackIndex, clipIndex);
		trimClipOut(trackIndex, clipIndex - 1, delta, false);
		notifyClipOut(trackIndex, clipIndex - 1);
	}

	void TimelineTracksModel::removeTransitionByTrimOut(int trackIndex, int clipIndex, int delta)
	{
	...
	bool TimelineTracksModel::trimTransitionInValid(int trackIndex, int clipIndex, int delta)
	{
		bool result = false;
		int i = m_trackList.at(trackIndex).mlt_index;
		QScopedPointer<Mlt::Producer> track(m_tractor->track(i));
	...
	void TimelineTracksModel::trimTransitionIn(int trackIndex, int clipIndex, int delta)
	{
		int i = m_trackList.at(trackIndex).mlt_index;
		QScopedPointer<Mlt::Producer> track(m_tractor->track(i));
		if (track) {
	...
	bool TimelineTracksModel::trimTransitionOutValid(int trackIndex, int clipIndex, int delta)
	{
		Q_UNUSED(delta)
			bool result = false;
		int i = m_trackList.at(trackIndex).mlt_index;
	...
	void TimelineTracksModel::trimTransitionOut(int trackIndex, int clipIndex, int delta)
	{
		int i = m_trackList.at(trackIndex).mlt_index;
		QScopedPointer<Mlt::Producer> track(m_tractor->track(i));
		if (track) {
	...
	bool TimelineTracksModel::addTransitionByTrimInValid(int trackIndex, int clipIndex, int delta)
	{
		Q_UNUSED(delta)
			bool result = false;
		int i = m_trackList.at(trackIndex).mlt_index;
	...
	void TimelineTracksModel::addTransitionByTrimIn(int trackIndex, int clipIndex, int delta)
	{
		int i = m_trackList.at(trackIndex).mlt_index;
		QScopedPointer<Mlt::Producer> track(m_tractor->track(i));
		if (track) {
	...
	bool TimelineTracksModel::addTransitionByTrimOutValid(int trackIndex, int clipIndex, int delta)
	{
		Q_UNUSED(delta)
			bool result = false;
		int i = m_trackList.at(trackIndex).mlt_index;
	...
	void TimelineTracksModel::addTransitionByTrimOut(int trackIndex, int clipIndex, int delta)
	{
		int i = m_trackList.at(trackIndex).mlt_index;
		QScopedPointer<Mlt::Producer> track(m_tractor->track(i));
		if (track) {
	...
	bool TimelineTracksModel::removeTransitionByTrimInValid(int trackIndex, int clipIndex, int delta)
	{
		bool result = false;
		int i = m_trackList.at(trackIndex).mlt_index;
		QScopedPointer<Mlt::Producer> track(m_tractor->track(i));
	...
	bool TimelineTracksModel::removeTransitionByTrimOutValid(int trackIndex, int clipIndex, int delta)
	{
		bool result = false;
		int i = m_trackList.at(trackIndex).mlt_index;
		QScopedPointer<Mlt::Producer> track(m_tractor->track(i));
	...
	void TimelineTracksModel::onFilterChanged(Mlt::Filter* filter)
	{
		if (filter && filter->is_valid()) {
	...
	bool TimelineTracksModel::moveClipToTrack(int fromTrack, int toTrack, int clipIndex, int position)
	{
		bool result;
		Mlt::Producer* trackFrom = m_tractor->track(m_trackList.at(fromTrack).mlt_index);
		Mlt::Playlist playlistFrom(*trackFrom);
	...
	void TimelineTracksModel::moveClipToEnd(Mlt::Playlist& playlist, int trackIndex, int clipIndex, int position)
	{
		int n = playlist.count();
		int length = position - playlist.clip_start(n - 1) - playlist.clip_length(n - 1);
	...
	void TimelineTracksModel::relocateClip(Mlt::Playlist& playlist, int trackIndex, int clipIndex, int position)
	{
		int targetIndex = playlist.get_clip_index_at(position);
	...
	void TimelineTracksModel::moveClipInBlank(Mlt::Playlist& playlist, int trackIndex, int clipIndex, int position)
	{
	...
	void TimelineTracksModel::consolidateBlanks(Mlt::Playlist &playlist, int trackIndex)
	{
		for (int i = 1; i < playlist.count(); i++) {
			if (playlist.is_blank(i - 1) && playlist.is_blank(i)) {
				int out = playlist.clip_length(i - 1) + playlist.clip_length(i) - 1;
	...
	void TimelineTracksModel::addBackgroundTrack()
	{
	...
	bool TimelineTracksModel::isTransition(Mlt::Playlist &playlist, int clipIndex) const
	{
		QScopedPointer<Mlt::Producer> producer(playlist.get_clip(clipIndex));
	...
	void TimelineTracksModel::insertTrack(int trackIndex, TrackType type)
	{
	...
	void TimelineTracksModel::addBlackTrackIfNeeded()
	{
		return;
	...
	Mlt::Transition *TimelineTracksModel::getTransition(const QString &name, int trackIndex) const
	{
	...
	void TimelineTracksModel::removeBlankPlaceholder(Mlt::Playlist& playlist, int trackIndex)
	{
